...
#define loratracker_COMMENT "MyName"
#define loratracker_MYCALL "NOCALL-9"
#define loratracker_SYMBOLTABLE '/'
#define loratracker_SYMBOLCHAR '>'
#define loratracker_DESTSSID 1
...
#define loratracker_SXRESET 14
#define loratracker_PTTLED 2
#define loratracker_MISO 19
#define loratracker_MOSI 27
#define loratracker_SCK 5
#define loratracker_NSS 18
#define loratracker_GPSRX 34
#define loratracker_GPSBAUD 9600
#define loratracker_UBATT 35
/* 30 */

#define loratracker_FASTBEACON 20        /* bacon intervall move, will be doubled after moveing until slowbeacon */
#define loratracker_SLOWBEACON 180       /* beacon intervall limit no move */
#define loratracker_SPEEDFASTBEACON 2.5  /* speed kmh to switch to fast beacon  */
#define loratracker_COMMENTINTERVALL 5   /* send 1 beacon with comment */
#define loratracker_BEACONFORMAT 2       /* 0 normal, 1 compressed, 2 mic-e */
...
static uint32_t beacontime;
static uint32_t commentcount;
static uart_t* uart2;
      ...
      if (b[1UL]=='G') {
         if ((b[3UL]=='R' && b[4UL]=='M') && b[5UL]=='C') {
            ...
            if (getnum(b, b_len, &i, len, &n)) {
               p->lat = p->lat+(double)(float)n/6.0f;
            }
            else return;
            if (getnum(b, b_len, &i, len, &n)) {
               p->lat = p->lat+(double)(float)n/60.0f;
            }
            else return;
            if (b[i]=='.') ++i;
            else return;
            if (getnum(b, b_len, &i, len, &n)) {
               p->lat = p->lat+(double)(float)n/600.0f;
            }
            else return;
            if (getnum(b, b_len, &i, len, &n)) {
               p->lat = p->lat+(double)(float)n/6000.0f;
            }
            else return;
            if (getnum(b, b_len, &i, len, &n)) {
               p->lat = p->lat+(double)(float)n/60000.0f;
            }
            if (getnum(b, b_len, &i, len, &n)) {
               p->lat = p->lat+(double)(float)n/6.E+5f;
            }
            skip(b, b_len, &i, len);
            ...
            if (getnum(b, b_len, &i, len, &n)) {
               p->long0 = p->long0+(double)(float)n/6.0f;
            }
            else return;
            if (getnum(b, b_len, &i, len, &n)) {
               p->long0 = p->long0+(double)(float)n/60.0f;
            }
            else return;
            if (b[i]=='.') ++i;
            else return;
            if (getnum(b, b_len, &i, len, &n)) {
               p->long0 = p->long0+(double)(float)n/600.0f;
            }
            else return;
            if (getnum(b, b_len, &i, len, &n)) {
               p->long0 = p->long0+(double)(float)n/6000.0f;
            }
            else return;
            if (getnum(b, b_len, &i, len, &n)) {
               p->long0 = p->long0+(double)(float)n/60000.0f;
            }
            if (getnum(b, b_len, &i, len, &n)) {
               p->long0 = p->long0+(double)(float)n/6.E+5f;
            }
            skip(b, b_len, &i, len);
...
static char scp(char rd, char nss, char sck, char mosi)
{
   char res;
   res = 0;
   if (rd) res = digitalRead(19UL);
   digitalWrite(27UL, mosi);
   digitalWrite(5UL, sck);
   digitalWrite(18UL, nss);
   return res;
} /* end scp() */
...
/* send or receive a buffer */
{
   ...
   while (j<len) {
      ...
      rd = 0UL;
      for (;;) {
         rd += rd+(uint32_t)scp(!wr && i<=7UL, i==0UL && j==len-1UL, 0,(((1 << (int)i) & (uint32_t)d) != 0));
         delayms();
         if (i==0UL) break;
         b = scp(0, 0, 1, (((1 << (int)i) & (uint32_t)d) != 0));
         delayms();
         --i;
      }
      if (!wr) s[j] = (char)rd;
...
static void setsynth(float mhz)
{
   ...
   synt = (uint32_t)truncc(mhz*16384.0f+0.5f);
...
Serial.print(m, HEX);
Serial.print("<>");
Serial.print(scpi(1UL), HEX);
Serial.print(" ");
      digitalWrite(loratracker_SXRESET, 0);   /* lora reset */
      delay(10);
      digitalWrite(loratracker_SXRESET, 1);
      delay(300);
      scpo(1UL, 0x80);
      delay(100);
      scpo(1UL, 0x81);
...
static void send(const char ftext[], uint32_t ftext_len)
{
   ...
   uint32_t i;
   char b[256];
   digitalWrite(loratracker_PTTLED, 1);
   Setmode(128UL, 1, 0);
   Setmode(129UL, 1, 0);
...
static void sendaprs(double lat, double long0, double alt, double course, double speed,
                char comp0, char withspd, char withalt, char dao, char com,
                const char mycall[], const char comm[], const char viapath[])
{
   char b[251];
   uint32_t micdest;
   uint32_t nl;
   ...
   if (viapath[0UL]) {
      Append(b, 251ul, ",", 2ul);
      Append(b, 251ul, viapath, strlen(viapath));
   }
   if (comp0==0UL) {
      /* uncompressed */
      Append(b, 251ul, ":!", 3ul);
      ...
      if (withspd) {
         ...
         ++i;
         b[i] = (char)(33UL+truncc((double)(log((float)(speed*5.3996146834962E-1+1.0))
                *1.29935872129E+1f)));
         ++i;
         b[i] = '_';
         ++i;
         ...
         if (alt*3.2808398950131>1.0) {
            n = truncc((double)(log((float)(alt*3.2808398950131))*500.5f));
         }
         else n = 0UL;
   ...
   if (com) {
      /*    beaconmacros(comm); */
      Append(b, 251ul, comm, strlen(comm));
   }
   /*  Append(b, CR+LF); */
//delay(100);
Serial.write("pr:");
...
//Serial.print(" adc:");
//Serial.print(analogRead(loratracker_UBATT));
Serial.print(" hall:");
...
Serial.print(" ");            
            ...
            if (pos.posok) {
               ++count;
               if (pos.speed >= loratracker_SPEEDFASTBEACON) beacontime = loratracker_FASTBEACON;     /* we move */
               if (count >= beacontime) {
                  sendaprs(pos.lat, pos.long0, pos.alt, pos.course, pos.speed, loratracker_BEACONFORMAT, 1, (pos.altok > 0), 0, (commentcount == 0),
                           loratracker_MYCALL, loratracker_COMMENT, "");
                  ++commentcount;
                  if (commentcount >= loratracker_COMMENTINTERVALL) commentcount = 0;
                  count = 0UL;
                  pos.altok = 0;                                    
                  if (pos.speed < loratracker_SPEEDFASTBEACON) {                                          /* we do not move so double beacon time */
                     beacontime = beacontime*2;
                     if (beacontime > loratracker_SLOWBEACON) beacontime = loratracker_SLOWBEACON;        /* limit beacon time */
                  }
               }
            }
         }
...
//void setpins(uart_t nuart, uart_t ntx, uart_t nrx)
//{
// uartSetPins(nuart, ntx, nrx);
//}
 
void setup()
{
   setCpuFrequencyMhz(20);
   micessid = loratracker_DESTSSID;
   symt = loratracker_SYMBOLTABLE;
   symb = loratracker_SYMBOLCHAR;
   sumoff = 0;
   ...
   gpsp = 0UL;
   commentcount = 0;
   beacontime = loratracker_FASTBEACON;
   pinMode(19UL, INPUT);
   pinMode(27UL, OUTPUT);
   pinMode(5UL, OUTPUT);
   pinMode(18UL, OUTPUT);
   pinMode(loratracker_SXRESET, OUTPUT);
   pinMode(loratracker_PTTLED, OUTPUT);
   digitalWrite(loratracker_SXRESET, 0);   /* lora reset */
   delay(100);
   digitalWrite(loratracker_SXRESET, 1);
   uart2 = uartBegin(2, loratracker_GPSBAUD, SERIAL_8N1, loratracker_GPSRX, -1, 256, false, 0);
   Serial.begin(115200UL);
Serial.write("Start");
}
void loop() {
  char c;
  if (uartAvailable(uart2) > 0) {
    c=uartRead(uart2);
//    c=Serial.read();  
    Serial.write(c); 
    gpsbyte(c);
  } else delay(10);
  if ((scpi(loratracker_RegIrqFlags)>>3) & 1) {
    scpo(18UL, 255UL); /* clear all irq */
    Setmode(0x81,1,0);
    digitalWrite(loratracker_PTTLED, 0);
  }
}

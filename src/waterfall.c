...
static void buttf(struct Complex * R, struct Complex * A,
                struct Complex * B)
{
   struct Complex h;
                ...
                char INVERS)
{
   ...
   */
   ...
   if (i<=tmp) for (;; i++) {
      { /* with */
         struct _4 * anonym = &REV[i];
         h = feld[anonym->y1];
         feld[anonym->y1] = feld[anonym->x];
         feld[anonym->x] = h;
      }
      if (i==tmp) break;
   } /* end for */
   ...
   /*
       FOR i:=1 TO N-1 DO
         feld[i].Re:=feld[i].Re*0.5;
         feld[i].Im:=feld[i].Im*0.5;
       END;
   */
   /*    wk:=-wk; */
   sko = FFTSIN[0U];
   ...
   if (z<=tmp) for (;; z++) {
      ...
      while (i<N) {
         h = feld[i+idd2];
         /*
               complex.csub(feld[i+idd2],feld[i],h);
               complex.cadd(feld[i],feld[i],h);
         */
         { /* with */
            struct Complex * anonym0 = &feld[i];
            feld[i+idd2].Re = anonym0->Re-h.Re;
            feld[i+idd2].Im = anonym0->Im-h.Im;
...
/*
  IF NOT INVERS THEN
    r:=1.0/FLOAT(N);
    feld[0].Re:=feld[0].Re*r;
    feld[0].Im:=feld[0].Im*r;

    r:=r*2.0;
    FOR i:=1 TO N-1 DO
      feld[i].Re:=feld[i].Re*r;
      feld[i].Im:=feld[i].Im*r;
    END;
  END;
*/
} /* end Transform() */
...
static void fft3(void)
{
   Transform(cp->Adr, cp->Len0, 0);
} /* end fft3() */
...
man, 1=Blackman-Harris, 2=Flat-Top", 84ul);
               osi_WrStrLn("                      3=Hamming, 4=Hann, 5=Rect",
                 48ul);
...
static void nowin(float w[], uint32_t w_len, float lev)
{
   uint32_t i;
   uint32_t tmp;
   lev = X2C_DIVR(1.0f,lev);
...
static void makewin(float w[], uint32_t w_len, float lev)
{
   ...
   else if (wintyp==5UL) nowin(w, w_len, lev);
...
/*
PROCEDURE chkfloat(VAR f:REAL);
VAR c:RECORD 
        CASE: CARD8 OF
          0:s:SET32;
         |1:b:ARRAY[0..3] OF CHAR;
         |2:f:REAL;
        END;
      END; 
    b:CHAR;
BEGIN
  MOVE(ADR(f), ADR(c.s), 4);
b:=c.b[0]; c.b[0]:=c.b[3]; c.b[3]:=b; 
b:=c.b[1]; c.b[1]:=c.b[2]; c.b[2]:=b; 
--b:=c.b[0]; c.b[0]:=c.b[1]; c.b[1]:=b; 
--b:=c.b[2]; c.b[2]:=c.b[3]; c.b[3]:=b; 
  IF c.s*SET32{24..30}=SET32{24..30} THEN c.f:=0.0;       (* real overflow *)
  ELSIF c.f>=2147483648.0 THEN c.f:=2147483648.0;
  ELSIF c.f<=-2147483648.0 THEN c.f:=-2147483648.0 END;
f:=c.f;
END chkfloat;
*/
...
{
   uint32_t s;
   X2C_MOVE((char *)f,(char *) &s,4UL);
   if ((s&0x7F000000UL)==0x7F000000UL) *f = 0.0f;
...
static char line(pINOUT c, uint32_t len, uint32_t jumpback)
{
   ...
   if (iqsize==2UL) {
      ...
      if (i<=tmp) for (;; i++) {
         { /* with */
            struct Complex * anonym0 = &c->Adr[i];
      ...
      if (i<=tmp) for (;; i++) {
         { /* with */
            struct Complex * anonym1 = &c->Adr[i];
   ...
   if (dcoffset) {
      ...
      if (i<=tmp) for (;; i++) {
         { /* with */
            struct Complex * anonym2 = &c->Adr[i];
            w = win->Adr[i]; /* window */
      ...
      if (i<=tmp) for (;; i++) {
         { /* with */
            struct Complex * anonym3 = &c->Adr[i];
            w = win->Adr[i]; /* window */
   ...
   if (i<=tmp) for (;; i++) {
      { /* with */
         struct Complex * anonym4 = &c->Adr[i];
   ...
   if (i<=tmp) for (;; i++) {
      ...
      c->Adr[i].Im = c->Adr[j].Re;
      if (i==tmp) break;
   } /* end for */
   if (dcoffset) {
      c->Adr[hsize-1UL].Im = (c->Adr[hsize].Im+c->Adr[hsize-2UL].Im)*0.5f;
                /* fill dc notch hole */
   }
   if (odc>0UL) {
      ...
      i = 0UL;
      if (i<=tmp) for (;; i++) {
         c->Adr[i].Im = X2C_DIVR(c->Adr[i].Im,
   ...
   if (zerolev!=0.0f) min0 = zerolev;
      ...
      i = mar;
      if (i<=tmp) for (;; i++) {
         hi = (int32_t)X2C_TRUNCI(c->Adr[i].Im-min0,X2C_min_longint,
                X2C_max_longint);
         if (hi<0L) hi = 0L;
...
extern int main(int argc, char **argv)
{
   ...
   checkfloat = 0;
   Parms();
   for (y = 0UL; y<=6143UL; y++) {
      IDDS[y] = (short)(int32_t)
                X2C_TRUNCI(32767.0f*osic_sin(X2C_DIVR((float)
                y*6.283185307f,8192.0f)),X2C_min_longint,X2C_max_longint);
   } /* end for */
   /*WrInt(IDDS[y], 10); */
   ...
   X2C_DYNALLOCATE((char **) &cp,sizeof(struct Complex),

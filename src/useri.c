...
aprsdecode_MONCALL useri_beaconimported;
unsigned long useri_beaconimporttime;
char useri_listwin;
...
static struct CONFIG configs[149];
...
{
   memset((char *)configs,(char)0,sizeof(struct CONFIG [149]));
} /* end clrconfig() */
...
extern void useri_sayonoff(char s[], unsigned long s_len, char on)
{
   ...
   useri_textautosize(0L, 0L, 4UL, 4UL, 'b', h, 100ul);
   X2C_PFREE(s);
} /* end sayonoff() */
...
static void dotoolsmenu(pMENU menu)
{
   ...
   /*  addonoff(menu, "   |Radiolink", CMDGEOPROFIL, MINH*5+17, 8,
                configon(fGEOPROFIL)); */
   /*
     addline(menu, " Rain Colormap", CMDRAIN, MINH*5+20);
     addline(menu, " Temp.Colormap", "W", MINH*5+25);
   */
   addline(menu, " WXonly|  All ", 15ul, "\300", 2ul, 520UL);
   addline(menu, " Temp  |  Rain", 15ul, "W", 2ul, 525UL);
   addline(menu, " Dimm Notmover", 15ul, "m", 2ul, 530UL);
...
static void helpmenu(void)
{
   ...
   addline(menu, "aprsmap(cu) 0.47 by OE5DXL ", 28ul, " ", 2ul, 605UL);
...
|\370\367|", 49ul, (char *) &myorbeacon, 1u/1u, 2100UL+16UL*y);
   } /* end for */
...
/* 1 on/off, 2 move to editline, 3 del, first n is 1 */
{
   ...
   pCONFLINE pl;
   pl = 0;
   if ((unsigned long)cfg<=148UL) pl = configs[cfg].lines;
   po = 0;
   while (n>1UL && pl) {
      po = pl;
      pl = pl->next;
...
extern void useri_encerr(char s[], unsigned long s_len)
{
   X2C_PCOPY((void **)&s,s_len);
   useri_textautosize(0L, 0L, 4UL, 4UL, 'e', s, s_len);
   X2C_PFREE(s);
} /* end encerr() */
...
static void beaconeditor(void)
{
   ...
   */
   ...
   if (useri_beaconimported[0UL]) {
      if (useri_beaconimported[0UL]=='\012') {
         useri_textautosize(0L, 0L, 5UL, 5UL, 'e', "Object Cloned to Beacon E\
ditor!", 32ul);
         useri_beaconimported[0UL] = 0;
      }
      else if (useri_beaconimporttime+2UL>TimeConv_time()) {
         strncpy(s,"Click again [",201u);
         aprsstr_Append(s, 201ul, useri_beaconimported, 9ul);
         aprsstr_Append(s, 201ul, "] to Clone Object", 18ul);
         useri_textautosize(0L, 0L, 5UL, 2UL, 'b', s, 201ul);
      }
   }
} /* end beaconeditor() */
   ...
   if (knob==1UL) {
      ...
      if (ch!='B') {
         ...
         if (ch=='m' || ch=='M') {
            ...
            useri_textautosize(0L, 0L, 4UL, 4UL, 'r', "encoding changed to \'\
Compressed\'", 33ul);
         }
      }
   }
...
static void beaconlen(void)
{
   ...
   aprsstr_Append(s, 201ul, h, 201ul);
   useri_textautosize(0L, 0L, 4UL, 4UL, 'b', s, 201ul);
} /* end beaconlen() */
...
/* do digi editing */
{
   ...
   if (caller==2U) {
      ...
      if (knob==1UL) {
         ...
         aprsstr_IntToStr((long)radius, 0UL, h, 200ul);
         icfg(useri_fDIGIRADIUS, h, 200ul);
         aprsstr_IntToStr((long)duptime, 0UL, h, 200ul);
         icfg(useri_fDIGITIME, h, 200ul);
      }
      else if (knob==2UL) {
         /* Delete */
...
static sCONFSET _cnst = {0x00000000UL,0x00000000UL,0x00407E80UL,0x00780000UL,
                0x0003C000UL};
...
static void configman(unsigned long button, char * keycmd)
{
   ...
   if (button==1UL) configedit = 0UL;
      ...
      /*    IF VAL(CONFSET, configedit) IN sCONFSET{fFONTSIZE, fANT1, fANT2,
                ...
                fCOLMARK2} */
      if (X2C_INL((long)configedit,149,_cnst)) *keycmd = ' ';
...
{
   configs[useri_fEDITLINE].width = 200U;
   if (knob==1UL) configedit = 147UL;
   else if (knob==2UL) configedit = 146UL;
   else if (knob==3UL) configedit = 119UL;
   else if (knob==4UL) configedit = 113UL;
   else if (knob==5UL) configedit = 114UL;
   else if (knob==6UL) configedit = 115UL;
   else if (knob==7UL) configedit = 118UL;
   else if (knob==8UL) configedit = 117UL;
   else configedit = 116UL;
   memcpy(configs[useri_fEDITLINE].title,configs[configedit].title,31u);
   copytoed();
   useri_rdlums();
   updatemenus();
...
static void docolourchoose(pMENU menu)
{
   ...
   ro = (unsigned long)useri_conf2int((unsigned char)((142UL+menu->scroll)
                -1UL), 0UL, 0L, 100L, 0L);
   go = (unsigned long)useri_conf2int((unsigned char)((142UL+menu->scroll)
                -1UL), 1UL, 0L, 100L, 0L);
   bo = (unsigned long)useri_conf2int((unsigned char)((142UL+menu->scroll)
                -1UL), 2UL, 0L, 100L, 0L);
   ...
   maptool_drawstri(menu->image, configs[(menu->scroll-1UL)+142UL].title,
                ...
                unsigned long cy)
{
   ...
   if (xytocolor(cx, 100UL-cy, &r, &g, &b, &alias)) {
      ...
      aprsstr_Append(s, 101ul, h, 101ul);
      icfg((unsigned char)((142UL+knob)-1UL), s, 101ul);
   }
   useri_rdlums();
   updatemenus();
} /* end setcolour() */
...
static void domap(unsigned long knob, unsigned long sub)
{
   ...
   */
   ...
   else if (knob==5UL) configtogg(useri_fTEMP);
   else if (knob==6UL) configtogg(useri_fWINDSYM);
   else if (knob==7UL) configtogg(useri_fARROW);
...
static void testbeep(unsigned char v)
{
   if (useri_configon(v)) {
      xosi_beep(100L, (unsigned long)useri_conf2int(v, 0UL, 20L, 8000L,
                ...
                250L));
      useri_textautosize(0L, 0L, 4UL, 1UL, 'b', "Testing beep now", 17ul);
   }
} /* end testbeep() */
...
static void configeditor(void)
{
   ...
   cnt = 0UL;
   pl = 0;
   if (configedit<=148UL) pl = configs[configedit].lines;
   ph = pl;
   while (ph) {
      ++cnt;
      ph = ph->next;
   ...
   if (pl) {
      ...
      if (sub>=1UL && sub<=3UL) {
         ...
         if (useri_guesssize(fn, 1024ul, h, 1024ul)>=0L) {
            /* there is a logfile */
            dellog = knob;
            useri_textautosize(0L, 0L, 4UL, 0UL, 'r', "Delele Logfile ? Click\
 \'Delete\'", 32ul);
         }
      }
         ...
         if (dellog==knob) {
            ...
            useri_textautosize(0L, 0L, 4UL, 0UL, 'r', h, 1024ul);
         }
         else if (dellog==0UL) configdelman(useri_fWATCH, 3UL, knob);
         else {
            useri_textautosize(0L, 0L, 4UL, 0UL, 'r', "logfile not deleted",
                20ul);
         }
         dellog = 0UL;
      }
...
static void tcpstat(void)
{
   ...
   useri_textautosize(0L, 0L, 4UL, 0UL, 'b', s, 1001ul);
} /* end tcpstat() */
...
static void op2watch(char add)
{
   ...
   if (op) {
      ...
      useri_textautosize(0L, 0L, 4UL, 4UL, 'b', s, 100ul);
   }
} /* end op2watch() */
   ...
   if (aprsdecode_click.entries>0UL) {
      { /* with */
         ...
         if (anonym->opf) {
            ...
            useri_textautosize(0L, 0L, 4UL, 4UL, 'r', s, 100ul);
         }
      }
   ...
   if (menu==0) {
      ...
      if (menu->wid==225UL) dolist(menu, potx, poty);
         ...
         if (useri_configon(useri_fINVMOV)) {
            if (aprsdecode_click.cmd=='\005') aprsdecode_click.cmd = '\030';
            else if (aprsdecode_click.cmd=='\030') {
               aprsdecode_click.cmd = '\005';
            }
         }
         aprsdecode_click.dryrun = 0;
         ...
         */
         aprsdecode_click.cmd = ' ';
         if (subknob==0UL) aprsdecode_lums.wxcol = 'W';
...
extern void useri_keychar(char ch, char ispasted, char movecmd)
{
   ...
   if (ch=='R') aprsdecode_click.cmd = '\022';
                ...
                && ch!='p') aprsdecode_click.cmd = X2C_CAP(ch);
   ...
   else if (aprsdecode_click.cmd=='B') dorfcfg(3UL, 0UL);
...
static void showexitcode(char name[], unsigned long name_len, long c)
{
   ...
   aprsstr_Append(s, 100ul, h, 100ul);
   useri_textautosize(0L, 0L, 4UL, 6UL, 'e', s, 100ul);
   X2C_PFREE(name);
} /* end showexitcode() */

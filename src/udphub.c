                ...
                unsigned long start)
{
   ...
   return ((unsigned long)(unsigned char)c[6U]/2UL&15UL)==((unsigned long)
                (unsigned char)b[start+6UL]/2UL&15UL);
} /* end cmpcall() */
                ...
                unsigned long buf_len)
{
   ...
   last = 0;
   while (u) {
      if (cmpcall(u->call, buf, buf_len, 7UL)) {
         if (last) {
            last->next = u->next;
            u->next = users;
            users = u;
...
static char FindIp(unsigned long * toip0, unsigned long * toport,
                const char buf[], unsigned long buf_len)
{
   pUSER u;
   u = users;
   while (u) {
      if (cmpcall(u->call, buf, buf_len, 0UL)) {
         *toip0 = u->uip;
         *toport = u->dport;
         return 1;
      }
...
extern int main(int argc, char **argv)
{
   ...
   lifetime = 604800UL;
   parms();
   if (digisock<0L) Err("need -u parameter", 18ul);
   users = 0;
   usersock = openudp();
   ...
   for (;;) {
      fdclr();
      fdsetr((unsigned long)digisock);
      fdsetr((unsigned long)usersock);
      if (selectrw(10UL, 0UL)>0L) {
         if (issetr((unsigned long)digisock)) {
            /* data from digi */
            fromip = digiip;
            ...
            if (blen>=17L) {
               ...
               if (FindIp(&toip, &userdport, ubuf, 338ul)) {
                  res = udpsend(usersock, ubuf, blen, userdport, toip);
               }
               else if (show) osi_WrStrLn(" digi out user not found", 25ul);
            }
         }
